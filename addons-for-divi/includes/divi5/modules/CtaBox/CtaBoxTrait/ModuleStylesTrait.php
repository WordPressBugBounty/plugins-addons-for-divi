...
trait ModuleStylesTrait
{
     ...
     *
     * PHP twin of buildCustomStyles() in src/divi5/modules/cta-box/styles.jsx.
    ...
    public static function build_custom_styles($advanced, $order_class)
    {
        ...
        $properties = [
            '--dtq-cta-maxw' => $advanced['contentWidth'] ?? [],
            '--dtq-cta-gap'  => $advanced['buttonGap'] ?? [],
        ];
        ...
        // The overlay colour only means anything while the overlay is on, and
        // it is not a responsive field, so it rides along with the desktop block.
        $use_overlay = ($advanced['useOverlay']['desktop']['value'] ?? 'off') === 'on';
        $overlay_raw = $advanced['overlayColor']['desktop']['value'] ?? '';
        if ($use_overlay && '' !== $overlay_raw) {
            $colour = dtq_css_color(dtq_resolve_css_value($overlay_raw), '');
            if ('' !== $colour) {
                $desktop[] = '--dtq-cta-overlay: ' . $colour . ';';
            }
        }
        ...
        foreach (['tablet', 'phone'] as $breakpoint) {
            ...
            if (!empty($declarations)) {
                $styles[] = [
                    'atRules'     => self::$dtq_cta_media[$breakpoint],
                    'selector'    => $order_class,
    ...
    public static function module_styles($args)
    {
        ...
        $all_styles = [
            ...
            // Eyebrow / headline / body typography.
