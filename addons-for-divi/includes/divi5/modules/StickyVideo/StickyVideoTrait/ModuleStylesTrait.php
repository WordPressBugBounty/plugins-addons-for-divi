<?php
/**
 * StickyVideo: Module styles trait.
 *
...
trait ModuleStylesTrait
{
     ...
     *
     * PHP twin of buildCustomStyles() in src/divi5/modules/sticky-video/styles.jsx.
    ...
    public static function build_custom_styles($advanced, $order_class)
    {
        ...
        foreach (['stickShadowColor' => '--dtq-sv-shadow'] as $key => $property) {
        ...
        $lengths = [
            'stickOffsetX' => '--dtq-sv-x',
            'stickOffsetY' => '--dtq-sv-y',
            'stickRadius'  => '--dtq-sv-radius',
        ];
        ...
        // Stack order is a bare integer, not a length, so dtq_css_length would
        // reject it outright.
        $z_index = $advanced['stickZIndex']['desktop']['value'] ?? '';
        if ('' !== $z_index && is_numeric(trim((string) $z_index))) {
            $desktop[] = '--dtq-sv-z: ' . (int) $z_index . ';';
        }
        ...
        foreach (['tablet', 'phone'] as $breakpoint) {
            ...
            if (!empty($declarations)) {
                $styles[] = [
                    'atRules'     => self::$dtq_sv_media[$breakpoint],
                    'selector'    => $order_class,
