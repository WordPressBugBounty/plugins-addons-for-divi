<?php
/**
 * ImageAccordion: Module styles trait.
 *
...
trait ModuleStylesTrait
{
     ...
     *
     * PHP twin of buildCustomStyles() in src/divi5/modules/image-accordion/styles.jsx.
    ...
    public static function build_custom_styles($advanced, $order_class)
    {
        ...
        $properties = [
            '--dtq-ia-h'   => $advanced['panelHeight'] ?? [],
            '--dtq-ia-gap' => $advanced['gap'] ?? [],
        ];
        ...
        // Non-responsive values ride along with the desktop block. The panels
        // inherit all of these from the parent's order class, which is how a
        // parent setting reaches child markup that knows nothing about it.
        foreach (['overlayColor' => '--dtq-ia-ov', 'overlayActiveColor' => '--dtq-ia-ov-active'] as $key => $property) {
        ...
        $speed = $advanced['speed']['desktop']['value'] ?? '';
        if ('' !== $speed) {
            $duration = dtq_css_length($speed, '');
            if ('' !== $duration) {
                $desktop[] = '--dtq-ia-speed: ' . $duration . ';';
            }
        }

        // Unitless, and it multiplies a flex factor rather than being a length.
        $grow = $advanced['growRatio']['desktop']['value'] ?? '';
        if ('' !== $grow && is_numeric(trim((string) $grow))) {
            $desktop[] = '--dtq-ia-grow: ' . (float) $grow . ';';
        }

        // Interpolated where CSS expects a timing function, so matched against
        // a list rather than pattern-checked.
        $easings = ['ease', 'ease-in', 'ease-out', 'ease-in-out', 'linear', 'cubic-bezier(0.4, 0, 0.2, 1)'];
        $easing  = $advanced['easing']['desktop']['value'] ?? '';
        if (in_array($easing, $easings, true)) {
            $desktop[] = '--dtq-ia-ease: ' . $easing . ';';
        }
        ...
        foreach (['tablet', 'phone'] as $breakpoint) {
            ...
            if (!empty($declarations)) {
                $styles[] = [
                    'atRules'     => self::$dtq_ia_media[$breakpoint],
                    'selector'    => $order_class,
    ...
    public static function module_styles($args)
    {
        ...
        $all_styles = [
            ...
            // Shared panel typography and chrome. These reach into the child
            // panels through the selectors declared in module.json.
